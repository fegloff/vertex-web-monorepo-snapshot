/**
 * TESTING: TradingView datafeed configuration temporarily disabled
 * This file contains mock types and configurations for testing purposes.
 * To re-enable full TradingView functionality, uncomment the original implementation
 * and remove the mock versions.
 */
...
} as Record<string, CandlestickPeriod>;
...
export const DATAFEED_CONFIGURATION = {
  supports_time: true,
  supported_resolutions: SUPPORTED_RESOLUTIONS, // ['1', '5', '15', '30', '60', '1D', '1W', '1M'] as ResolutionString[],
};
...
export function getTradingViewSymbolInfo(
  ...
  const symbolInfo = ((): TradingViewSymbolInfo => {
     ...
     * and remove the mock versions.
     */
    const priceNumDecimalPlaces = 10;
    // precisionFixed(
    //   market.priceIncrement.toNumber(),
    // );
    const priceScale = 10 ** priceNumDecimalPlaces;
    const minMov = 1; // market.priceIncrement.multipliedBy(priceScale).toNumber();
  ...
  const decimalPlaces = 10; // market.priceIncrement.decimalPlaces();
